...
void date_time_str(FILETIME* ft, char* buf, size_t buf_size)
{
    SYSTEMTIME st = {0};
    buf[0] = 0;
    ...
    if (ft->dwHighDateTime == 0 && ft->dwLowDateTime == 0)    
        ...
        _snprintf(buf, buf_size, "%4d.%02d.%02d %02d:%02d:0%d.%03d", 
...
void set_list_view_columns (HWND hwnd, int list_view_id, SColumns* cols, int col_nums)
{
    ...
    {   	
		...
		SendDlgItemMessage (hwnd, list_view_id, LVM_INSERTCOLUMN, (WPARAM) i, (LPARAM)&lvcolumn);
    } 
...
void __stdcall ORDER_STATUS_CALLBACK(
				long nMode
				, DWORD dwTransID
				, EntityNumber orderNum
				, LPCTSTR ClassCode
				...
				, OrderDescriptor descriptor)
{
	...
	sprintf (item_text, "%I64d", orderNum);
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;
	...
	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_DATE(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_TIME(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_ACTIVATION_TIME(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_WITHDRAW_TIME(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_EXPIRY(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_ORDER_ACCRUED_INT(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_ORDER_YIELD(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_UID(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_ORDER_USERID(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_ORDER_ACCOUNT(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_ORDER_BROKERREF(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_ORDER_CLIENT_CODE(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_ORDER_FIRMID(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;
    ...
    sprintf (item_text, "%ld", TRANS2QUIK_ORDER_PERIOD(descriptor));
    SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    sprintf (item_text, "%ld", TRANS2QUIK_ORDER_DATE_TIME(descriptor, ORDER_MICROSEC));
    SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    FILETIME ft = TRANS2QUIK_ORDER_FILETIME(descriptor);
    date_time_str(&ft, item_text, sizeof(item_text));
    SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    sprintf (item_text, "%ld", TRANS2QUIK_ORDER_DATE_TIME(descriptor, ORDER_WITHDRAW_QUIKDATE));
    SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    sprintf (item_text, "%ld", TRANS2QUIK_ORDER_DATE_TIME(descriptor, ORDER_WITHDRAW_MICROSEC));
    SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    ft = TRANS2QUIK_ORDER_WITHDRAW_FILETIME(descriptor);
    date_time_str(&ft, item_text, sizeof(item_text));
    SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_VALUE_ENTRY_TYPE(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
	lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_EXTENDED_FLAGS(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
	lvitem.iSubItem++;
	...
	sprintf (item_text, "%ld", TRANS2QUIK_ORDER_EXEC_TYPE(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
	lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_ORDER_AWG_PRICE(descriptor));
	SendMessage (orders_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
	lvitem.iSubItem++;
...
void __stdcall TRADE_STATUS_CALLBACK (
	long nMode
	, EntityNumber tradeNumber
	...
	, TradeDescriptor descriptor)
{
	...
	sprintf (item_text, "%I64d", tradeNumber);
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%I64d", orderNumber);
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;
	...
	sprintf (item_text, "%ld", TRANS2QUIK_TRADE_SETTLE_DATE(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_TRADE_DATE(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_TRADE_TIME(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_TRADE_IS_MARGINAL(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_ACCRUED_INT(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_YIELD(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_CLEARING_CENTER_COMMISSION(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_EXCHANGE_COMMISSION(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_TS_COMMISSION(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_TRADING_SYSTEM_COMMISSION(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_PRICE2(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_REPO_RATE(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_REPO2_VALUE(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_ACCRUED_INT2(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_TRADE_REPO_TERM(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_START_DISCOUNT(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_LOWER_DISCOUNT(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_UPPER_DISCOUNT(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%ld", TRANS2QUIK_TRADE_BLOCK_SECURITIES(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_CURRENCY(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_SETTLE_CURRENCY(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_SETTLE_CODE(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_ACCOUNT(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_BROKERREF(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_CLIENT_CODE(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_USERID(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_FIRMID(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_PARTNER_FIRMID(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_EXCHANGE_CODE(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%s", TRANS2QUIK_TRADE_STATION_ID(descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    sprintf (item_text, "%ld", TRANS2QUIK_TRADE_PERIOD (descriptor));
    SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    sprintf (item_text, "%ld", TRANS2QUIK_TRADE_DATE_TIME(descriptor, TRADE_MICROSEC));
    SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    FILETIME ft = TRANS2QUIK_TRADE_FILETIME(descriptor);
    date_time_str(&ft, item_text, sizeof(item_text));
    SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

    sprintf (item_text, "%ld", TRANS2QUIK_TRADE_KIND (descriptor));
    SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
    lvitem.iSubItem++;

	sprintf (item_text, "%lf", TRANS2QUIK_TRADE_BROKER_COMMISSION (descriptor));
	SendMessage (trades_list_view, LVM_SETITEMTEXT, (WPARAM) Item, (LPARAM)&lvitem);
	lvitem.iSubItem++;
...
UINT ASyncTransThread (LPVOID pvParam)
{
	...
	{
		// TODO: Add your control notification handler code here
		TRANS2QUIK_SEND_ASYNC_TRANSACTION (pszTransaction, &nExtendedErrorCode, szErrorMessage, sizeof (szErrorMessage));		
		Sleep(100);	
	}
